...
class UsuarioRepository:
    ...
    def anonimizar_email(self, usuario_id: int) -> None:
        ...
        Libera o UNIQUE constraint para novo cadastro com o mesmo email.
...
class TransacaoRepository:
    ...
    def listar_por_periodo(self, data_inicio: str, data_fim: str, usuario_id: int) -> list[dict]:
        with self._db.get_conn() as conn:
            rows = conn.execute(
                """SELECT t.id, t.uuid, t.descricao, t.valor, t.tipo,
    ...
    def gastos_por_categoria(self, ano: int, mes: int, usuario_id: int) -> list[dict]:
        with self._db.get_conn() as conn:
            rows = conn.execute(
    ...
    def evolucao_saldo_mes(self, ano: int, mes: int, usuario_id: int) -> list[dict]:
        with self._db.get_conn() as conn:
            rows = conn.execute(
    ...
    def existe_recorrente_no_mes(self, recorrente_uuid: str, ano: int, mes: int) -> bool:
        with self._db.get_conn() as conn:
            count = conn.execute(
...
class RecorrenteRepository:
    ...
    def inserir(self, uuid, descricao, valor, tipo, categoria_id, dia_vencimento, usuario_id) -> int:
        with self._db.get_write_conn() as conn:
            cur = conn.execute(
                """INSERT INTO recorrentes
                   (uuid, descricao, valor, tipo, categoria_id, dia_vencimento, usuario_id)
                   VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (uuid, descricao.strip(), round(valor, 2), tipo, categoria_id, dia_vencimento, usuario_id),
            )
            return cur.lastrowid
    ...
    def buscar_por_uuid(self, uuid: str, usuario_id: int) -> dict | None:
        with self._db.get_conn() as conn:
            row = conn.execute(
                """SELECT id, uuid, descricao, valor, tipo, categoria_id, dia_vencimento
                   FROM recorrentes
...
class MetaRepository:
    ...
    def inserir(self, uuid, titulo, valor_alvo, data_inicio, usuario_id,
                descricao="", data_fim=None, categoria_id=None) -> int:
        with self._db.get_write_conn() as conn:
            cur = conn.execute(
                """INSERT INTO metas
...
class LimiteCategoriaRepository:
    ...
    def listar(self, usuario_id: int) -> list[dict]:
        with self._db.get_conn() as conn:
            rows = conn.execute(
                """SELECT lc.categoria_id, lc.limite, c.nome, c.icone, c.cor
...
class BuscaRepository:
    ...
    def buscar(self, usuario_id, termo="", conta_id=None, tipo=None,
               data_inicio=None, data_fim=None, limit=100) -> list[dict]:
...
class SaldoContaRepository:
    ...
    def saldos_por_conta(self, usuario_id: int) -> list[dict]:
        with self._db.get_conn() as conn:
            rows = conn.execute("""
                SELECT cb.id, cb.nome, cb.tipo, cb.icone,
                       COALESCE(SUM(
        ...
        with self._db.get_conn() as conn:
            rows = conn.execute(
                """SELECT id, nome, tipo, icone FROM contas_bancarias
                   WHERE usuario_id = ? AND ativo = 1 ORDER BY nome""",
                (usuario_id,)
            ).fetchall()
        return _rows_to_list(rows)
